...
################################## Option ####################################
if Option==1:
    drifter_ID = 154420701#152300811 
    # if raw data, use "drift_X.dat";if want to get drifter data in database, use "None"
    INPUT_DATA =  'drift_su_2014_1.dat'#'drift_jml_2015_1.dat'      
...
if Option == 1:
    ...
    print "Drifter: %s forecast %d days"%(drifter_ID,track_days)
    start_time = datetime.now(pytz.UTC)-timedelta(1) #datetime(2015,1,24,0,0,0,0,pytz.UTC)
    ...
    print "drifter points: ",len(dr_points['lon']),'(',dr_points['lat'][-1],',',dr_points['lon'][-1],')'
    ...
    if MODEL=='FVCOM':
        ...
        b_points = get_obj.get_data(url_fvcom2) # b_points is model boundary points.
